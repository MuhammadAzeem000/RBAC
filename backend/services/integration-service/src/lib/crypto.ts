...
// AES-256-GCM encrypt/decrypt for connector credential blobs — an explicit,
// documented stand-in for a real secrets manager (Vault/cloud KMS), not one
// itself (see prisma/schema.prisma's ConnectorCredential comment). The key
// is derived from CREDENTIAL_ENCRYPTION_KEY via SHA-256 so any sufficiently
// long passphrase works as the env var, not just a raw 32-byte value.
...
// ConnectorCredential.encryptedSecret.
