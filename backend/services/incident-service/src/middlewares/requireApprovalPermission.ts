...
async function run(req: Request, res: Response, next: NextFunction, actionName: string) {
  ...
  const approvalsModule = permissions.find((p) => p.name === MODULE_NAMES.APPROVALS);
  if (!approvalsModule?.actions.includes(actionName)) {
...
// Mirrors requireIncidentPermission — same pattern, different module.
export function requireApprovalPermission(actionName: string) {
