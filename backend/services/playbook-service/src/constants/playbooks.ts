...
// escalationAfter fires a best-effort Slack notification to
// escalationChannel if the approval sits undecided that long, before
// finally expiring at timeoutDuration if it's still undecided.
...
// Seed data only — the runtime catalog is the DB-backed Playbook /
...
// serves it). `requiresApproval` is a denormalized display flag —
...
// are still simulated actions (no real connector bound) but run as real,
// individually-retried, individually-audited Temporal Activities (see
// src/temporal/) — each step becomes its own StepExecution row.
// `enrich-ioc`'s last two steps are real: they set `connector`/`action` and
// execute through integration-service's connector runtime against the live
// Slack/VirusTotal APIs — see temporal/activities.ts's runStep(). The
// VirusTotal lookup targets a fixed demo IP (8.8.8.8) since there's no real
// alert-entity pipeline binding a step's params to an actual incident's
// indicators yet.
