...
import { THREAT_INTEL_EXCHANGE, THREAT_INTEL_IOC_INGESTED_ROUTING_KEY } from "../events/threatIntelTopology";

const SERVICE_NAME = "threat-intelligence-service";
const POLL_INTERVAL_MS = 2000;
const BATCH_SIZE = 20;
const CONNECT_RETRY_DELAY_MS = 5000;

// Event types that need to reach a future business consumer, not just the
// audit trail — see the dual-publish comment in publishBatch() below.
const DOMAIN_EVENT_ROUTING_KEYS: Record<string, string> = {
  IOC_INGESTED: THREAT_INTEL_IOC_INGESTED_ROUTING_KEY,
};
...
// front so a row needing the domain-event fan-out never fails because the
// second exchange wasn't declared yet. Identical pattern to
// alert-ingestion-service's dual-publish outboxPublisher.
async function connectPublisher(): Promise<void> {
  ...
  for (;;) {
    try {
      ...
      await channel.assertExchange(THREAT_INTEL_EXCHANGE, "topic", { durable: true });
...
async function publishBatch(): Promise<void> {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      // event types a future domain consumer depends on — same durable
      ...
      // delivered).
      const domainRoutingKey = DOMAIN_EVENT_ROUTING_KEYS[row.event_type];
      if (domainRoutingKey) {
        await publishConfirmed(confirmChannel, THREAT_INTEL_EXCHANGE, domainRoutingKey, body);
      }
