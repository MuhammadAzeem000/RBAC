...
const envSchema = z.object({
  ...
  // AES-256-GCM key for encrypting TAXII server credentials at rest (see
  // src/lib/crypto.ts) — same explicit stand-in for a real secrets manager
  // (Vault/cloud KMS) that integration-service uses, not a shared secret
  // with it (each service validates its own copy of this env var).
  CREDENTIAL_ENCRYPTION_KEY: z.string().min(32),
  // Shared-secret header checked by requireServiceToken.ts, guarding the
  // pre-auth IOC lookup route a future connector/enrichment step calls
  // machine-to-machine (no user JWT exists in that context).
  THREAT_INTELLIGENCE_SERVICE_TOKEN: z.string().min(16),
});
