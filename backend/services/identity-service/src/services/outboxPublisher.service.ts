...
import { AUDIT_EXCHANGE } from "../events/topology";
...
// the broker has actually accepted the message (persisted it, for a
// durable exchange/queue), satisfying "publisher confirms where supported".
// If the broker is unreachable at all, this just keeps retrying in the
// background; outbox rows stay unpublished (durable in this service's own
// database) until it reconnects — the outage never loses anything.
 ...
 * (`FOR UPDATE SKIP LOCKED`) inside one transaction — this is what makes it
 * safe to run more than one instance of this worker at once: two workers
 * racing this query never see the same row, so neither can double-publish
 * the same outbox row (the eventId-idempotent consumer downstream is a
 * second, independent line of defense, not a substitute for this one).
 */
  ...
  for (const row of rows) {
    ...
    try {
      ...
      await prisma.outboxEvent.update({ where: { id: row.id }, data: { lastError: errorMessage } }).catch(() => {
        // Best-effort: if even the error-recording update fails (DB blip),
        // the row simply stays unpublished with its previous attempts count
        // and gets picked up again next poll — nothing is lost.
      });
    }
  }
}
